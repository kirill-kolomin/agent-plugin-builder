...
2. **Which component types?** Skills and MCP servers are both optional, though a plugin without either supplies no portable components. Most requests are skills-only; add `mcp.json` only if the user actually has server(s) to expose.
